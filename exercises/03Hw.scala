...
object Hw03Task1 {
  ...
  /** Consider the language of arithmetic expressions with `With`, (see
    * https://ps-tuebingen-courses.github.io/pl1-lecture-notes/05-name-binding/name-binding.html)
    * extended with two new binding constructs as illustrated by the following
    * abstract syntax:
    */
  enum Exp:
    case Num(n: Int)
  ...
  /** The purpose of the `Let` construct is to bind a list of identifiers in
    * such a way that the scope of the bound variables is only the body, but not
    * any of the right-hand sides of definitions. In particular, there is no
    * shadowing between the definitions. For instance, the following test case
    * should evaluate to 7 and not to 11:
    */

  val test1 =
    With("x", 1, Let(List("x" -> 5, "y" -> Add("x", 1)), Add("x", "y")))

  /** Subtasks:
    *
    * 1) Implement the missing part of the `eval` and `subst` function to
    * support `Let`. Only change the parts currently filled with an error!
    *
    * 2) There is some redundancy in the binding constructs of this language.
    * Eliminate the construct `With` by defining it as syntactic sugar.
    *
  ...
  val test2 =
    With("x", 1, LetStar(List("x" -> 5, "y" -> Add("x", 1)), Add("x", "y")))

  /** Your task: Implement the missing parts of `subst` and `eval` to support
    * `LetStar`. (Again, only change the parts currently filled with an error!)
...
object Hw03Task2 {
  ...
  /** Consider the environment-based interpreter for the language `F1WAE` from
    * the lecture notes
    * (https://ps-tuebingen-courses.github.io/pl1-lecture-notes/06-first-order-functions/first-order-functions.html):
    */
  ...
  def evalWithEnv(funs: Funs, env: Env, e: Exp): Int = e match {
    ...
    case Call(f, args) => {
      ...
      val vargs =
        args.map(evalWithEnv(funs, env, _)) // evaluate function arguments
  ...
  def evalDynScope(funs: Funs, env: Env, e: Exp): Int = e match {
    ...
    case With(x, xdef, body) =>
      evalDynScope(funs, env + ((x, evalDynScope(funs, env, xdef))), body)
    case Call(f, args) => {
      val fd = funs(f)
      ...
      if (fd.args.size != vargs.size)
        sys.error("number of paramters in call to " + f + " does not match")
      val newenv =
        env ++ fd.args.zip(vargs) // extending env instead of Map() !!
      evalDynScope(funs, newenv, fd.body)
    }
  }

  /** The following example from the lecture notes yields a result with the
    * dynamically scoped interpreter, but fails with the statically scoped
    * interpreter.
    */
  ...
  /** Your task: Explain why this is the case. What is the difference between
    * static scoping and dynamic scoping? What is the problem with dynamic
    * scoping?
    */
}
