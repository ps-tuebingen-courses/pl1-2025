...
object DefunGADT {
  ...
  def flatMap_defun(
      xs: List[Int]
  )(f: FunctionInt[List[(Int, Int)]]): List[(Int, Int)] =
    xs match {
      case Nil     => Nil
...
assert(
  product(List(1, 2, 3), List(3, 4, 5)) == DefunGADT.product_defun(
    List(1, 2, 3),
    List(3, 4, 5)
  )
)
...
assert(
  product(List(1, 2, 3), List(3, 4, 5)) == DefunTwo.product_defun(
    List(1, 2, 3),
    List(3, 4, 5)
  )
)
